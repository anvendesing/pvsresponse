...
const EMPTY: FormState = { slug: "", name: "", sortOrder: 0, active: true };
...
export const CategoryManager = () => {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const list = await api.productCategories();
      setRows(list);
    } catch (e) {
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await api.updateProductCategory(editing.id, {
          slug: form.slug.trim().toLowerCase(),
          name: form.name.trim(),
          sortOrder: form.sortOrder,
          active: form.active,
        });
        setBanner(`Category "${form.name}" updated.`);
      } else {
        ...
        setBanner(`Category "${form.name}" created.`);
      }
      setModalOpen(false);
      await load();
    } catch (e) {
  ...
  const remove = async (c: ProductCategory) => {
    ...
    if (count > 0) {
      setError(`Cannot delete: ${count} product(s) still use "${c.name}".`);
      return;
    }
    ...
    try {
      ...
      setBanner(`Category "${c.name}" deleted.`);
      await load();
    } catch (e) {
      setError((e as Error).message);
    } finally {
  ...
  const uploadImage = async (c: ProductCategory, file: File) => {
    setUploadingId(c.id);
    setError(null);
    try {
      await api.uploadCategoryImage(c.id, file);
    ...
    <div className="space-y-4">
      ...
      >
        <p className="text-body-sm text-ink-muted px-4 pb-3 border-b border-border">
          Categories appear on the ecommerce home page and in the product editor dropdown.
          Deactivate to hide from the storefront without deleting.
        </p>
        {loading ? (
              ...
              <div key={c.id} className="px-4 py-3 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-caption text-ink-muted font-mono">
                    /category/{c.slug} · sort {c.sortOrder}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-[60] bg-ink/40 grid place-items-center" onClick={() => setModalOpen(false)}>
          ...
          >
            <div className="px-5 py-3 border-b border-border flex justify-between items-center">
              <div className="font-bold">{editing ? "Edit category" : "New category"}</div>
