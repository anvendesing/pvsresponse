        ...
        <>
          ...
          <div className="mobile-filter-bar">
            ...
            <div className="mobile-sort-row">
              <label htmlFor="concern-sort-select" className="sr-only">Sort by</label>
              <select
            ...
            <>
              ...
              <div className="filter-sheet" role="dialog" aria-modal="true" aria-label="Filters">
                <div className="filter-sheet__header">
                  ...
                  <button type="button" className="filter-sheet__close" onClick={() => setFilterOpen(false)} aria-label="Close filters">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="filter-sheet__body">
                  ...
                  <div className="filter-sheet__check-group">
                    <CheckboxRow checked={showInStock} label="In Stock" onToggle={() => { setShowInStock((v) => !v); setPage(1); }} />
                    <CheckboxRow checked={showOutOfStock} label="Out of Stock" onToggle={() => { setShowOutOfStock((v) => !v); setPage(1); }} />
                  </div>
                </div>

                <div className="filter-sheet__footer">
                  <button type="button" className="btn" onClick={() => { setShowInStock(true); setShowOutOfStock(false); setSortOrder("default"); setPage(1); }}>
                    Reset
                  </button>
        ...
        <div className="listing-row">
          ...
          <div className="listing-grid-area">
            ...
            {productsLoading ? (
              ...
              <div className="card-soft" style={{ textAlign: "center", padding: "3rem 1.5rem" }}>
                <p className="muted">No products tagged for this concern yet. Check back soon.</p>
              </div>
            ) : (
              <div className="listing-products-grid">
                {visible.map((p) => <ProductCard key={p.id} product={p} />)}
...
const Pagination = ({
  ...
  <div className="pagination">
    <button type="button" className="pagination-btn" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} aria-label="Previous page">
      <ChevronLeftIcon />
    </button>
    {Array.from({ length: pageCount }).map((_, i) => (
      <button key={i} type="button" className={`pagination-btn ${i + 1 === page ? "active" : ""}`} onClick={() => setPage(i + 1)}>
        {i + 1}
      </button>
    ))}
    <button type="button" className="pagination-btn" onClick={() => setPage((p) => Math.min(pageCount, p + 1))} disabled={page === pageCount} aria-label="Next page">
      <ChevronRightIcon />
    </button>
  </div>
);
